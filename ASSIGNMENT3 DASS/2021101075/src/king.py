...
class King:
    ...
    def attack_target(self, target,attack,King):
        if(self.alive == False):
            return
    ...
    def specialAttack(self,V,King):
        if(self.alive == False):
            return
        options = V.get_attack_options(self.position,self.attack_radius)
        for pos in options:
            self.attack_target(options[pos],self.AoE,King)

    def normalAttack(self,V,King):
        if(self.alive == False):
            return
        a = self.position[0]
        b = self.position[1]
        ...
        if V.map[a][b] != pt.BLANK and V.map[a][b] != pt.SPAWN:
            ...
            self.attack_target(target,self.attack,King)
...
class Queen:
    ...
    def specialAttack(self,V,King):
        if(self.alive == False):
            return
        r = Timer(self.specialAttackDelay,self.specialAttackfn,(V,King))
        try:
            r.start()
        except:
            exit()
        
    def specialAttackfn(self,V,King):
        a = self.position[0]
        b = self.position[1]
        if self.facing == 'up':
            a -= 16
        ...
        for i in range(a,a+9):
            for j in range(b,b+9):
                ...
                if V.map[i][j] != pt.BLANK and V.map[i][j] != pt.SPAWN:
                    ...
                    self.attack_target(target,self.AoE,King)
