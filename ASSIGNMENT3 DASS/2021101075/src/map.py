...
import numpy as np
import datetime
# import character of stealth archer as well
...
# here define the color of the stealth archer as the energy decrease

def stealtharchColor(sarch):
    health = sarch.health
    max_health = sarch.max_health
    percentage = (health*100)/max_health
    if(percentage > 50):
        return Back.LIGHTMAGENTA_EX
    elif percentage > 20:
...
def printMap(V):
    ...
    # here handle the way stealth archer will come written onto the game 
  
    for sarch in stealth_archers:
        a= 2*sarch.position[0]
        b = 2*sarch.position[1]
        map_matrix[a][b] = stealtharchColor(sarch) + Fore.BLACK + 'ST'
        map_matrix[a+1][b] = stealtharchColor(sarch) + Fore.BLACK + '  '
        map_matrix[a][b+1] = stealtharchColor(sarch)+ Fore.BLACK + 'AR'
        map_matrix[a+1][b+1] = stealtharchColor(sarch) + Fore.BLACK + 'CH'
    ...
    for hlr in healers:
        ...
        map_matrix[a+1][b] = hlrColor(hlr) + Fore.BLACK + 'AL'
        map_matrix[a][b+1] = hlrColor(hlr) + Fore.BLACK + 'LL'
